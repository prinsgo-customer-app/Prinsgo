...
// Update with your real support contact number (with country code, no + or spaces for tel/whatsapp links)
export const SUPPORT_PHONE = '9186299995010';
...
⚠️ **If you deployed an earlier version of this app and saw a blank white/black screen on web**: that was caused by a bug in `vercel.json`'s rewrite rule (it was redirecting the JS bundle file itself back to `index.html`, so the app's JavaScript never actually loaded). This is fixed in this version — the rewrite now correctly excludes files with extensions (`.js`, `.css`, etc.) and only redirects route-like paths.
